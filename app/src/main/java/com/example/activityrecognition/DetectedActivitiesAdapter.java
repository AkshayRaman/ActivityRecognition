...
class DetectedActivitiesAdapter extends ArrayAdapter<DetectedActivity> {
    ...
    void updateActivities(ArrayList<DetectedActivity> detectedActivities) {
        SparseIntArray detectedActivitiesMap = new SparseIntArray();
        ...
        for (int i = 0; i < Constants.MONITORED_ACTIVITIES.length; i++) {
            int confidence = detectedActivitiesMap.get(Constants.MONITORED_ACTIVITIES[i], 0);
            Log.d("Activity", Utils.getActivityString(this.getContext(), Constants.MONITORED_ACTIVITIES[i]) + " : " + confidence);
        ...
        tempList.add(new DetectedActivity(Constants.MONITORED_ACTIVITIES[activity_index],
                max_conf));
